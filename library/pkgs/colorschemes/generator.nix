...
let
  inherit (pkgs) lib;
  inherit (lib) isString genAttrs importJSON concatStringsSep;
  ...
  isHexColor = c:
    isString c && (builtins.match "#([0-9a-fA-F]{3}){1,2}" c) != null;
  ...
  generateColorscheme = name: source:
    ...
    in pkgs.runCommand "colorscheme-${name}" {
      ...
      in {
        inherit schemeTypes;
        imported =
          genAttrs schemeTypes (scheme: importJSON "${drv}/${scheme}.json");
      };
    } ''
      mkdir "$out" -p
      for type in ${concatStringsSep " " schemeTypes}; do
        ${matugen}/bin/matugen ${
